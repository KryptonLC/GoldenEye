...
from lunar_symbols import read_lunar_symbols, get_lunar_data, save_lunar_data

# Step 2: Initialize key_usage dict
key_usage = read_key_usage()

# Step 3: Predefine time parameter pairs
time_pairs = [
...
# Step 4: Loop through symbols and time pairs
def landing_process():
    # Step 1: Read lunar symbols and order by last_update ascending
    result_code, symbols_df = read_lunar_symbols()

    # Ensure the DataFrame is ordered by symbol_id in ascending order
    ...
    for index, row in symbols_df.iterrows():
        ...
        for start_time, end_time in time_pairs:
            ...
            print(f"{symbol_id} | {symbol_name} | {start_time}:{end_time} | {key_usage['key_outlook']['minute']}/10 | {key_usage['key_outlook']['day']}/2000")

            # Measure time for save_lunar_data
            start_time_save = time.time()
            if not data_df.empty:
                save_result_code, save_message = save_lunar_data(data_df)
                print(f"Data saved for {symbol_name} ({symbol_id}) from {start_time} to {end_time}: {save_message}")
            else:
                print(f"No data returned for {symbol_name} ({symbol_id}) from {start_time} to {end_time}. Skipping save.")
